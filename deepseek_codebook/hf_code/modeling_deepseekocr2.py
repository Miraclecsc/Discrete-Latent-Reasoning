        ...
        if sam_model is not None and (input_ids.shape[1] != 1 or self.training) and torch.sum(images[0][1]).item() != 0:
            ...
            for image, crop_shape in zip(images, images_spatial_crop):
                ...
                # with torch.inference_mode(): 
                    ...
                    # 开始保存embedding：
                    import time
                    os.makedirs("big_files/top_right_feature", exist_ok=True)
                    cpu_features = [feat.cpu() for feat in images_in_this_batch]
                    torch.save(cpu_features, f"big_files/top_right_feature/feature_after_qwen2_{int(time.time() * 1000)}.pt")
                    # 结束保存embedding。
