...
PROJECTOR_PATH="${PROJECTOR_PATH:-$PROJECT_ROOT/outputs/latent_pretrain/latent/projector.pt}"
TRAIN_DATA_PATH="${TRAIN_DATA_PATH:-$PROJECT_ROOT/data/latent_sft.jsonl}"
OUTPUT_DIR="${OUTPUT_DIR:-$PROJECT_ROOT/outputs/latent_sft}"
...
SAVE_STEPS="${SAVE_STEPS:-94}"
SAVE_TOTAL_LIMIT="${SAVE_TOTAL_LIMIT:-10}"
OPTIM="${OPTIM:-adamw_torch}"
OPTIM_ARGS="${OPTIM_ARGS:-foreach=False}"
LR_SCHEDULER_TYPE="${LR_SCHEDULER_TYPE:-cosine}"
FREEZE_BASE_MODEL="${FREEZE_BASE_MODEL:-false}"
FREEZE_CODEBOOK="${FREEZE_CODEBOOK:-false}"
BF16="${BF16:-true}"

echo "=== Latent SFT 8-GPU Launch ==="
